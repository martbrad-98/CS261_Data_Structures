...
int bst_size_helper(struct bst_node* n){
  int size = 0;
  ...
  if(n == NULL){
    ...
    size = (bst_size_helper(n->left) + bst_size_helper(n->right) + 1);
  }
}
 ...
 */
int bst_height_helper(struct bst_node* n){
  int max1 = 1;
  int max2 = 1;
  if(n == NULL){
    return -1;
    ...
    max1 = bst_height_helper(n->left);
    max2 = bst_height_helper(n->right);
    if(max1 < max2){
      return (max2 + 1);
    }
    else{
...
int bst_height(struct bst* bst) {
  ...
  height = bst_height_helper(bst->root);
...
int bst_path_sum_helper(int sum, struct bst_node* n){
  int lsum = 0;
  ...
  if(n==NULL){
    ...
    lsum = sum - n->val;
    if(lsum == 0 && n->left == NULL && n->right == NULL){
      return 1;
    }
      return (bst_path_sum_helper(lsum, n->left) || bst_path_sum_helper(lsum, n->right));
  }
...
int bst_path_sum(int sum, struct bst* bst) {
  ...
  psum = bst_path_sum_helper(sum, bst->root);
...
void bst_node_iterator(struct bst_node* n, struct stack* s){
  if(n == NULL){
    return ;
  }

  bst_node_iterator(n->left, s);
  stack_push(s, n->val);

  bst_node_iterator(n->right, s);
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  ...
  i->stack = stack_create();

  bst_node_iterator(bst->root, i->stack);
  i->stack = reverse(i->stack);
